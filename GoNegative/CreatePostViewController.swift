...
class CreatePostViewController: UIViewController , UITextViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate{
    ...
    @IBOutlet weak var postTitle: KMPlaceholderTextView!
    ...
    override func viewDidLoad() {
        ...
        postText.font = UIFont(name: "AvenirNext-Regular" , size: 18)
        postTitle.delegate = self
        postTitle.font = UIFont(name: "AvenirNext-Medium" , size: 20)
...
//        postText.inputAccessoryView = toolBarView
    ...
    @IBAction func submitPost(_ sender: UIBarButtonItem) {
        ...
        if postTitle.text == ""
        {
            CDAlertView(title: "Title Field Blank", message: "We require a fabulous title to submit this post.", type: .error).show()
            return
        }
        if postImage.image == nil
        {
            CDAlertView(title: "No Image Selected", message: "We definiely need an Image to submit this post.", type: .error).show()
            return
        }
        if postText.text == ""
        {
            CDAlertView(title: "Story Field Blank", message: "It's all about stories, mate. Please write something in the story feild.", type: .error).show()
            return
        }
        ...
        let uploadTask = ImageRef.put(data!, metadata: nil) { (metadata, error) in
            ...
            ImageRef.downloadURL(completion: { (url, error) in
                ...
                if let url = url{
                    let feed = ["userID":uid,"postImageURL":url.absoluteString,"storyText": self.postText.text,"likes": 0,"author": UserDefaults.standard.string(forKey: "userName")!,"postDate": postDate,"postID": key,"postTitle": self.postTitle.text,"urlToAuthorImage": UserDefaults.standard.string(forKey: "userProfileImageURL")!] as [String : Any]
